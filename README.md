...
On first launch from the installer build, the app bootstraps backend runtime dependencies (`backend/requirements-runtime.txt`), then starts the studio and pre-pulls the default model in the background. To skip model pre-pull for testing, set `DOLPHINPHOTO_SKIP_MODEL_PULL=1`.
